...
@Suite struct EventLoggerTests {
    ...
    @Test func reportWritesSegmentAndConsoleLine() async throws {
        ...
        let seg = Segment(source: .mic, timestamp: Date(timeIntervalSince1970: 0), duration: 1.0, text: "hi")
    ...
    @Test func happyPathSequenceHasCorrectEventOrder() async throws {
        ...
        await logger.stateChanged(.loadingModel)
        await logger.stateChanged(.capturing)
        let seg = Segment(source: .mic, timestamp: Date(timeIntervalSince1970: 1_700_000_000), duration: 1.0, text: "hi")
        await logger.report(seg)
        ...
        if case .stateChanged(_, let d) = events[1] { #expect(d.state == .loadingModel) } else { Issue.record("events[1] != state_changed") }
        if case .stateChanged(_, let d) = events[2] { #expect(d.state == .capturing) } else { Issue.record("events[2] != state_changed") }
        #expect(events[3].typeString == "segment")
        if case .stateChanged(_, let d) = events[4] { #expect(d.state == .stopping) } else { Issue.record("events[4] != state_changed") }
        if case .sessionStopped(_, let d) = events.last! { #expect(d.reason == .sigint) } else { Issue.record("last != session_stopped") }
    }
    ...
    @Test func errorPathSequenceOmitsStoppingAndReportsReasonError() async throws {
        ...
        // `state_changed: stopping` は発行されないこと(spec line 95: SIGINT 経由のみ)を検証する。
        let url = tempURL()
        defer { try? FileManager.default.removeItem(at: url) }
        let writer = try JSONLWriter(output: url)
        let stream = InMemoryStream()
    ...
    @Test func nonSegmentEventsUseEmitTimeTimestamp() async throws {
        // spec line 52 の「segment 以外は Date() 発行時点」契約を、残り 4 methods でも pin する。
