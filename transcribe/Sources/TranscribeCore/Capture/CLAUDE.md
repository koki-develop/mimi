...
**No fire-and-forget `Task { reporter.* }` in this module.** All non-fatal capture-layer messages flow via `AsyncStream<CaptureDiagnostic>` (synchronous yield from `AudioOutputTap`) to be drained by `Pipeline.run`'s diagnostic task. This prevents shutdown races where actor hops would write to a closed `JSONLWriter`.
