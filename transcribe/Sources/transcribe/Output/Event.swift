...
extension Event: Codable {
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(ISO8601Format.shared.string(from: timestamp), forKey: .timestamp)
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let ts = ISO8601Format.shared.date(from: tsString) else {
            throw DecodingError.dataCorruptedError(
                forKey: .timestamp,
                in: container,
        ...
        switch kind {
        case .sessionStarted:
            self = .sessionStarted(timestamp: ts, data: try container.decode(SessionStartedData.self, forKey: .data))
        case .stateChanged:
            self = .stateChanged(timestamp: ts, data: try container.decode(StateChangedData.self, forKey: .data))
        case .segment:
        ...
        case .sessionStopped:
            self = .sessionStopped(timestamp: ts, data: try container.decode(SessionStoppedData.self, forKey: .data))
        }
    }
}
