    ...
    func finish() throws -> [Float] {
        ...
        while true {
            ...
            case .inputRanDry, .endOfStream:
                return collected
            case .error:
                throw AudioSampleConverterError.conversionFailed("converter returned error status while draining")
            @unknown default:
                throw AudioSampleConverterError.conversionFailed("converter returned unknown status while draining")
            }
        }
    }
