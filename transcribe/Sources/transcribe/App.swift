...
public struct App {
    public static func run(output: URL, modelName: String, verbose: Bool = false) async throws {
        ...
        do {
            ...
            try await fail(.unexpected(String(describing: error)), message: "Permission check failed: \(error)")
        }
        ...
        do {
            ...
            try await fail(.model(String(describing: error)), message: "Failed to load model '\(modelName)': \(error)")
        }
        ...
        let micTranscriber = Transcriber(source: .mic, model: micModel, reporter: logger, verbose: verbose)
        let systemTranscriber = Transcriber(source: .system, model: systemModel, reporter: logger, verbose: verbose)
        let counter = Counter()
