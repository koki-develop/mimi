...
文字起こしの品質について:
- <transcript> は自動文字起こしの結果であり、誤認識・同音異義語の誤変換・固有名詞の崩れ・助詞の欠落・意味の通らない短い断片の混入などが含まれることがあります
- 前後の文脈から発話意図が明らかな箇所は、文脈に沿って自然に解釈し直してよい（例: 明らかな同音異義語の取り違えは正しい語に寄せる）
- 意味が取れない断片・明らかなノイズは、無理に拾わず要約から落とす

出力要件:
- 今回の <transcript> 区間で起きた内容に焦点を絞って要約する（previous_entries はあくまで文脈参照用）
- 日本語 1 段落、4〜6 文、全体で 200〜400 字程度
- 会話の流れがわかる連文で書く（箇条書き禁止）
- 発話の趣旨に忠実に。文字起こしに無い話題や結論を作らない（事実の創作禁止）
